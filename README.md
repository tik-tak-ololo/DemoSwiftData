...
- `shoppingList` и `product` — ссылки на список и товар.

Ссылки на родителей имеют тип optional, поскольку при удалении и обработке графа объектов SwiftData связь может временно отсутствовать. Инициализатор и store-слой при обычной работе всегда создают позицию одновременно со списком и товаром.
